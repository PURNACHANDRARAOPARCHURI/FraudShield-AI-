...
export const HumanReviewConsole: React.FC<HumanReviewConsoleProps> = ({ transactions, onRefresh }) => {
  ...
  return (
    <div className="glass-panel rounded-2xl p-6 border border-slate-800 space-y-6">
      <div className="flex flex-col md:flex-row md:items-center justify-between pb-4 border-b border-slate-800 gap-4">
        <div>
          <div className="flex items-center space-x-2">
            ...
            <h2 className="text-lg font-bold text-white">Human Review Security Triage</h2>
          </div>
        ...
        <div className="flex items-center space-x-2">
          <span className="px-3 py-1 bg-rose-950/80 text-rose-400 border border-rose-800 text-xs font-bold rounded-full">
      ...
      {msg && (
        <div className="p-3 rounded-xl bg-cyan-950/60 border border-cyan-800 text-xs text-cyan-300 font-medium">
          {msg}
        </div>
      )}
      ...
      {flagged.length === 0 ? (
        <div className="text-center py-12 bg-slate-900/40 rounded-2xl border border-slate-800/80">
          ...
          <h3 className="text-sm font-bold text-white">Triage Queue Clear!</h3>
          <p className="text-xs text-slate-400 mt-1">Zero pending high-risk transactions requiring manual intervention.</p>
        </div>
      ) : (
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          {/* LIST OF FLAGGED TRANSACTIONS */}
          <div className="space-y-3">
            <h3 className="text-xs font-bold uppercase tracking-wider text-slate-400">Flagged Queue</h3>
              ...
              {flagged.map((tx) => (
                ...
                >
                  <div className="flex justify-between items-center text-xs">
                    <span className="font-mono font-bold text-white">{tx.transaction_id}</span>
                    <span className="text-rose-400 font-bold">{(tx.overall_risk_score * 100).toFixed(0)}% Risk</span>
                  </div>
          ...
          {selectedTx && (
            <div className="lg:col-span-2 glass-card p-5 rounded-2xl border border-slate-800 space-y-4">
              <div className="flex justify-between items-center pb-3 border-b border-slate-800">
                <div>
                  <h4 className="text-sm font-bold text-white">Transaction Detail Audit</h4>
                  <span className="text-xs font-mono text-cyan-400">{selectedTx.transaction_id}</span>
                </div>
                <span className="px-2.5 py-1 text-xs font-bold rounded-lg bg-rose-950 text-rose-300 border border-rose-800">
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-3 text-xs">
                ...
                <div className="p-2.5 rounded-lg bg-slate-900 border border-slate-800">
                  <span className="text-[10px] text-slate-400 block">Sender Account</span>
                  <span className="font-mono text-slate-200">{selectedTx.sender_account}</span>
                </div>
                <div className="p-2.5 rounded-lg bg-slate-900 border border-slate-800">
                  <span className="text-[10px] text-slate-400 block">Receiver Account</span>
                  <span className="font-mono text-slate-200">{selectedTx.receiver_account}</span>
                </div>
                <div className="p-2.5 rounded-lg bg-slate-900 border border-slate-800">
              ...
              <div>
                ...
                <textarea
                  ...
                  className="w-full p-2.5 bg-slate-950 border border-slate-800 rounded-xl text-xs text-white focus:outline-none focus:ring-2 focus:ring-cyan-500"
