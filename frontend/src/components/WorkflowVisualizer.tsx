    ...
    <div className="glass-panel rounded-2xl p-6 border border-slate-800 relative overflow-hidden">
      ...
      <div className="flex items-center justify-between pb-4 border-b border-slate-800">
        <div className="flex items-center space-x-2">
          ...
          <h3 className="text-lg font-bold text-white">LangGraph Multi-Agent Execution Graph</h3>
        </div>
        <div className="flex items-center space-x-2 text-xs font-mono">
          ...
          <span className="text-slate-400">{loading ? 'PIPELINE RUNNING...' : (isEvaluated ? 'EXECUTION COMPLETE' : 'STANDBY MODE')}</span>
        </div>
      </div>
      ...
      <div className="mt-6 space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 relative">
          ...
          <div className={`p-4 rounded-xl border transition-all ${loading ? 'border-cyan-500/50 bg-cyan-950/20' : 'border-slate-800 bg-slate-900/60'}`}>
            ...
            <h4 className="text-sm font-semibold text-white">Transaction Data Input</h4>
            <p className="text-[11px] text-slate-400 mt-1">5 Required + 7 Optional features normalized into 7D feature vector</p>
          </div>

          {/* Step 2: Dual ML Models */}
          <div className={`p-4 rounded-xl border transition-all ${isEvaluated ? 'border-cyan-500/80 bg-slate-900 shadow-lg shadow-cyan-500/10' : 'border-slate-800 bg-slate-900/60'}`}>
            <div className="flex items-center justify-between text-xs mb-2">
              <span className="font-bold text-indigo-400 uppercase tracking-wider">Step 3: Dual ML Models</span>
          ...
          <div className={`p-4 rounded-xl border transition-all ${isFastPath ? 'border-emerald-500 bg-emerald-950/20' : 'border-amber-500/60 bg-slate-900/60'}`}>
            ...
            <p className="text-[11px] text-slate-300 mt-1">
              {isFastPath ? '⚡ Fast Path: Risk < 0.40 (Instant Approve)' : '⚠️ Risk &ge; 0.40: Dispatched to LangGraph Orchestrator'}
            </p>
          </div>
        </div>

        {/* ROW 2: LANGGRAPH 4 PARALLEL AGENTS CONTAINER */}
        <div className={`p-5 rounded-2xl border transition-all ${loading ? 'border-cyan-500 bg-cyan-950/10 animate-pulse' : 'border-slate-800 bg-slate-950/60'}`}>
          <div className="flex items-center justify-between mb-4">
            <div className="flex items-center space-x-2">
              ...
              <h4 className="text-sm font-bold text-white">LangGraph Parallel Multi-Agent Orchestrator</h4>
            </div>
            <span className="text-xs text-slate-400 font-mono">4 Autonomous Agents</span>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
            ...
            <div className="p-3.5 rounded-xl bg-slate-900 border border-slate-800 hover:border-cyan-500/50 transition-colors">
              <div className="flex items-center justify-between mb-1">
                <span className="text-[11px] font-bold text-cyan-400">Agent 1</span>
                <span className="text-xs font-bold text-white">{isEvaluated ? (response.agent_scores.behavior_score * 100).toFixed(0) + '%' : '--'}</span>
              </div>
              <h5 className="text-xs font-semibold text-slate-200">Behavior Analysis</h5>
              <p className="text-[10px] text-slate-400 mt-1 line-clamp-2">Velocity, hardware fingerprint, geo shift</p>
            </div>

            {/* Agent 2 */}
            <div className="p-3.5 rounded-xl bg-slate-900 border border-slate-800 hover:border-indigo-500/50 transition-colors">
              <div className="flex items-center justify-between mb-1">
                <span className="text-[11px] font-bold text-indigo-400">Agent 2</span>
                <span className="text-xs font-bold text-white">{isEvaluated ? (response.agent_scores.historical_score * 100).toFixed(0) + '%' : '--'}</span>
              </div>
              <h5 className="text-xs font-semibold text-slate-200">Historical Fraud</h5>
              <p className="text-[10px] text-slate-400 mt-1 line-clamp-2">PostgreSQL chargeback & failed OTP history</p>
            </div>

            {/* Agent 3 */}
            <div className="p-3.5 rounded-xl bg-slate-900 border border-slate-800 hover:border-purple-500/50 transition-colors">
              <div className="flex items-center justify-between mb-1">
                <span className="text-[11px] font-bold text-purple-400">Agent 3</span>
                <span className="text-xs font-bold text-white">{isEvaluated ? (response.agent_scores.knowledge_score * 100).toFixed(0) + '%' : '--'}</span>
              </div>
              <h5 className="text-xs font-semibold text-slate-200">Knowledge Retrieval</h5>
              <p className="text-[10px] text-slate-400 mt-1 line-clamp-2">ChromaDB vector search on RBI & AML rules</p>
            </div>

            {/* Agent 4 */}
            <div className="p-3.5 rounded-xl bg-slate-900 border border-slate-800 hover:border-rose-500/50 transition-colors">
              <div className="flex items-center justify-between mb-1">
                <span className="text-[11px] font-bold text-rose-400">Agent 4</span>
                <span className="text-xs font-bold text-white">{isEvaluated ? (response.agent_scores.rule_score * 100).toFixed(0) + '%' : '--'}</span>
              </div>
              <h5 className="text-xs font-semibold text-slate-200">Rule Engine</h5>
              <p className="text-[10px] text-slate-400 mt-1 line-clamp-2">$10k limit, midnight transfer, foreign IP</p>
            </div>
          </div>
        </div>

        {/* ROW 3: RISK AGGREGATOR -> LLM JUDGE -> DECISION */}
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          {/* Aggregator */}
          <div className="p-4 rounded-xl bg-slate-900 border border-slate-800">
            <div className="flex items-center justify-between text-xs text-slate-400 mb-1">
              <span className="font-bold text-cyan-400 uppercase tracking-wider">Step 9: Risk Aggregator</span>
              <Activity className="w-4 h-4 text-cyan-400" />
            </div>
            <h4 className="text-xs font-semibold text-white">Weighted Risk Matrix</h4>
            <div className="mt-2 text-center py-2 bg-slate-950 rounded-lg border border-slate-800">
              <span className="text-xs text-slate-400 block">Enterprise Score</span>
          ...
          <div className="p-4 rounded-xl bg-slate-900 border border-slate-800">
            <div className="flex items-center justify-between text-xs text-slate-400 mb-1">
              <span className="font-bold text-purple-400 uppercase tracking-wider">Step 11: LLM-as-a-Judge</span>
              <Scale className="w-4 h-4 text-purple-400" />
            </div>
            <h4 className="text-xs font-semibold text-white">Guardrails & Hallucinations</h4>
          ...
          <div className={`p-4 rounded-xl border flex flex-col justify-between ${
            ...
            <span className="text-xs font-bold uppercase tracking-wider text-slate-300">Step 12: Decision Gate</span>
            <div className="my-2">
            ...
            <span className="text-[10px] text-slate-400">
              ...
              {response?.final_decision === 'OTP_REQUIRED' && '0.40 - 0.75 -> OTP Challenge Sent'}
              {response?.final_decision === 'HUMAN_REVIEW' && 'Risk > 0.75 -> Flagged for Human Review'}
            </span>
          </div>
        </div>
      </div>
